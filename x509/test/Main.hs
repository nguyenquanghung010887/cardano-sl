...
main = runQuickCheck
    ...
    , quickCheckResult $ label "validateSHA256 fails for unknown ServiceID" propUnknownService
...
propServerCertsInvalid (ExtExtendedKeyUsage purposes) =
    if any (== KeyUsagePurpose_ClientAuth) purposes then
        const (property True)
    else
        (=/= [])
...
generateAndValidate getServiceID predicate (tlsConf, dirConf) = do
    ...
    fmap conjoin $ forM certDescs $ \desc -> do
        ...
        predicate extendedKeyUsage <$>
            validateSHA256 caCert (certChecks desc) (getServiceID tlsConf) cert
